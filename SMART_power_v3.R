...
list_reg_all <- seq(1, B) %>% 
  parallel::mclapply(\(x) {
    sim_list <- sim_4t_microrand(N = c(50, 100, 200), capacity = 10)
    
    list(sim_list$lmer_reg %>% mutate(r = x),
         sim_list$summary %>% mutate(r = x))
  }, mc.cores = 10, mc.set.seed = T)

# saveRDS(list_reg_all, 'list_reg_all_B2000_cap10_v2.rds')


list_reg_all <- readRDS("list_reg_all_B2000_cap10_v2.rds")
